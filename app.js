...
let img = new Image();
let drawing = false;
let history = []; // pile des états du masque (ImageData)
...
selRal.value = RAL.find(c=>c.code==='RAL 7016').hex;
...
function setupCanvasForImage(image){
  // base logique fixe pour qualité export
  const W = 1280, H = 720;
  [photoCanvas, maskCanvas, outCanvas].forEach(c=>{
    c.width = W; c.height = H;
    c.style.width = '100%'; c.style.height = '100%';
  });
  pCtx = photoCanvas.getContext('2d');
...
/* ====== Baguette magique (flood fill couleur proche) ====== */
...
function magicWandSelect(sx, sy){
  ...
  // On dessinera sur un buffer masque pour éviter de dégrader la pile en direct
  ...
  // Adoucissement des bords (feather simple par flou box)
...
// Flou boîte rapide sur alpha du masque pour "feather"
...
function renderAll(){
  ...
  // mélange multiply sur la photo de sortie
...
function applyFinish(ctx, W, H, fin){
  // Mat / satiné : ajustements subtils via voile
...
/* ====== Tip: re-render on window resize (visual only) ====== */
